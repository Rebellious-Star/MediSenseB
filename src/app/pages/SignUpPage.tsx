...
import { signup, getStoredUser } from "../api/auth";
...
export function SignUpPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      console.log("SignUpPage: Starting direct signup...");
      const result = await signup({ name, email, password });
      console.log("SignUpPage: Signup successful:", result);
      
      // Verify user is stored
      const storedUser = getStoredUser();
      console.log("SignUpPage: User stored check:", storedUser);
      
      if (!storedUser) {
        console.error("SignUpPage: ERROR - User not stored in localStorage!");
          ...
          <CardContent className="p-8">
            ...
            <form onSubmit={handleSignup} className="space-y-6">
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full bg-gradient-to-r from-emerald-600 to-sky-600 hover:from-emerald-700 hover:to-sky-700 text-white font-medium py-3 px-4 rounded-lg transition-all duration-200 flex items-center justify-center space-x-2"
              >
                {loading ? (
